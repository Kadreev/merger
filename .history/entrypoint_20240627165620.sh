...
# Remove the config directory from the staged changes
git reset HEAD config/

# Add the specific files that we want to commit, ignoring errors for non-existent files
...
# Display the status after staging
...
if [[ -z $(git status -s) ]]; then
  ...
  # Display the status after committing
  echo "Status Check: Post Commit"
